...
- `internal/store/migrations` — numbered, checksum-verified schema migrations. `verifySchemaVersion` (run by both `migrate()` and `store.OpenForBackup`) refuses to proceed when `MAX(schema_migrations.version)` exceeds the highest bundled migration — an older binary won't silently run against, or back up, a schema a newer build created. `store.New` = `open` + `migrate`; `store.OpenForBackup` = `open` + `verifySchemaVersion` only.
...
- New periodic background job → append a `scheduledJob{name, interval, run}` to the slice in `startWorker` (`cmd/dockvmap/worker.go`); each gets its own goroutine, `time.Timer` and `select` — never share one `select` across jobs. Set `onShutdown` too if the job must do a final action on a clean stop. The `name` is the `worker_ticks` key — keep it stable and unique. A new service the job needs goes on the `workerDeps` struct, not a new positional arg.
