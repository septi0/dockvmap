...
`cmd/dockvmap/worker.go`. `serve()` calls `scheduledJobs(deps workerDeps)` once (synchronously) to build the full `[]scheduledJob`, then `jobDescriptors(jobs)` → `service.NewWorkerCatalog(...)` for the web layer, then runs the set in a goroutine via `runScheduledJobs`, which spawns each *enabled* job through `runScheduledJob`: **one goroutine, one `time.Timer`, one `select` per job**; never share one `select` across jobs.
...
- `executeJob` writes the tick (`MarkRun`, via `context.WithoutCancel` + 5s) **before** calling `run`, so a job that always fails still advances instead of retrying every wake; then `MarkOutcome` (fresh `context.WithoutCancel` + 5s) **after**, recording the count and error text into `worker_ticks`. It also `recover()`s so one job's panic can't take down the worker.
...
- `trigger` (`<-chan struct{}`, nil unless the job can run early on demand). A job registers its channel by calling `deps.trigger.Register(name)` in `scheduledJobs`; that call is the single source of "which jobs are triggerable" (`jobDescriptors` reads `trigger != nil`). `service.WorkerTrigger` has no fixed job list; `Register` creates the channel on first call, `Trigger(name)` returns false for anything never registered. Used by `image-tag-refresh` (the "refresh all tags" GUI button) and the two disk-reclaiming blob-cache jobs (System > Tasks "Run now"); `POST /api/system/tasks/{name}/run` is the generic trigger endpoint.
