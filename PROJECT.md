...
- New periodic background job → append a `scheduledJob{name, interval, run}` to the slice in `startWorker` (`cmd/dockvmap/worker.go`); each gets its own goroutine, `time.Timer` and `select` — never share one `select` across jobs. The `name` is the `worker_ticks` key — keep it stable and unique.
