...
| `tag_filters_path` | Path to a `filters.yaml` policy file (see Tag filtering below). If set, the file must exist, and an invalid path is a startup error, not a silent fallback. Unset uses the built-in default filters |
